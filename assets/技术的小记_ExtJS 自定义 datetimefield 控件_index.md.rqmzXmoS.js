import{_ as s,c as a,o as p,a8 as e}from"./chunks/framework.Cc4yYURl.js";const l="/assets/image.BBydLIMm.png",o=JSON.parse('{"title":"ExtJS 自定义 datetimefield 控件","description":"","frontmatter":{"createDate":"2016/05/07","title":"ExtJS 自定义 datetimefield 控件"},"headers":[],"relativePath":"技术的小记/ExtJS 自定义 datetimefield 控件/index.md","filePath":"技术的小记/ExtJS 自定义 datetimefield 控件/index.md","lastUpdated":1756407495000}'),i={name:"技术的小记/ExtJS 自定义 datetimefield 控件/index.md"};function r(c,n,b,t,m,u){return p(),a("div",null,n[0]||(n[0]=[e(`<h1 id="extjs-自定义-datetimefield-控件" tabindex="-1">ExtJS 自定义 datetimefield 控件 <a class="header-anchor" href="#extjs-自定义-datetimefield-控件" aria-label="Permalink to &quot;ExtJS 自定义 datetimefield 控件&quot;">​</a></h1><p>2016年05月07日</p><p>这一年都在使用 ExtJS 5.0 开发系统，经常会遇到的一个场景就是时间日期的选择了。比如说，预定场地的时候就必须由用户选择预定的日期和时间。</p><p>可惜的是，ExtJS 5.0 没有自带一个可以同时选择日期时间的控件（ExtJS 6.0 已经推出了，还没用过，不知道加入这种控件没~），通常偷懒的做法就是，先放一个日期控件，然后再放一个时间控件。用户使用的时候，先选择日期，再选择时间，最后用很别扭的办法将日期和时间拼装起来。</p><p>理想的状态是，提供给用户一个可以同时选择时间和日期的控件，并且数据是“日期+时间”的 Date 类型。既符合使用习惯，又方便开发。</p><p>故第二种偷懒的方式是，从网上扒一个现成的解决方案来用。此法最快，但是在过程中遇到了一个问题，现成可以搜索到的方案都有点旧，大多数还停留在 ExtJS 2.X、3.X 和 4.X 版本，自从 ExtJS 从 4 升级到 5 之后，API 改变了许多，所以并不能直接使用。</p><p>最后只能是自己写一个。准确来说，应该是拿了一份现成的代码修改，这个代码在 ExtJS 5.0 下不能正确运行，但是需要修改的量非常少，所以很感谢这份现成代码的作者，可惜没有署名，不知道是谁。</p><p>ExtJS 5.0 上测试成功，实现方法是：</p><h4 id="_1-自定义一个时间选择器-timepickerfield-继承自-ext-form-field-base-由三个-numberfield-组成。" tabindex="-1">1. 自定义一个时间选择器 TimePickerField，继承自 Ext.form.field.Base，由三个 NumberField 组成。 <a class="header-anchor" href="#_1-自定义一个时间选择器-timepickerfield-继承自-ext-form-field-base-由三个-numberfield-组成。" aria-label="Permalink to &quot;1. 自定义一个时间选择器 TimePickerField，继承自 Ext.form.field.Base，由三个 NumberField 组成。&quot;">​</a></h4><div class="language- vp-adaptive-theme line-numbers-mode"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>  Ext.define(&#39;MyApp.ux.DateTimeField.TimePickerField&#39;, {</span></span>
